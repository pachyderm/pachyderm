...
1. Run the corresponding steps for your operating system:
   ...
   * For a Debian-based Linux 64-bit or Windows 10 or later running on
   WSL:
     ...
     ```
     $ curl -o /tmp/pachctl.deb -L https://github.com/pachyderm/pachyderm/releases/download/v1.9.6/pachctl_1.9.6_amd64.deb && sudo dpkg -i /tmp/pachctl.deb
     ```

   * For all other Linux flavors:

     ```
     $ curl -o /tmp/pachctl.tar.gz -L https://github.com/pachyderm/pachyderm/releases/download/v1.9.6/pachctl_1.9.6_linux_amd64.tar.gz && tar -xvf /tmp/pachctl.tar.gz -C /tmp && sudo cp /tmp/pachctl_1.9.6_linux_amd64/pachctl /usr/local/bin
     ```
