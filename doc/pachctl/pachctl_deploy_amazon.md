...
<id>, <secret>, and <token> are session token details, used for authorization. You can get these by running 'aws sts get-session-token'
      ...
      --vault-address string             The address/hostport at which a Hashicorp Vault server can be reached, where pachd can read AWS credentials
      --vault-role string                The role that has been configured in the vault s3 secret engine for pachd (determines the vault path at which pachd reads AWS creds)
      --vault-token string               The token that pachd should use when requesting AWS credentials from vault
```
