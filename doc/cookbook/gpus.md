...
Pachyderm has alpha support for utilizing GPUs within Pachyderm pipelines (e.g., for training machine learning models). And just like CPU and RAM, Pachyderm relies on Kuberentes to serve the GPU resources to the pod. Therefore, to leverage GPUs in Pachyderm, they first need to be set up in Kubernetes. To do this, you will need to:

1. [Deploy a GPU enabled pachyderm cluster](gpus.html#deploy-a-gpu-enabled-pachyderm-cluster)
2. [Create a Docker image that is able to utilize GPUs](gpus.html#creating-a-gpu-enabled-docker-image)
3. [Write a pipeline spec that specifies GPU nodes](gpus.html#writing-your-pipeline-specification)

_Note: If you already have a GPU enabled Kubernetes cluster, you can skip Step 1 and proceed to [Step 2](gpus.html#creating-a-gpu-enabled-docker-image)._

Once you have a GPU enabled Kubernetes cluster, you can check out a concrete Pachyderm GPU example in our [example Tensorflow pipeline](https://github.com/pachyderm/pachyderm/tree/master/doc/examples/ml/tensorflow), which includes a pipeline specification for running model training on a GPU node.

## Step 1. Deploy a GPU Enabled Pachyderm Cluster
...
**Node** -  If you upped your `rootVolumeSize` (and set the `rootVolumeType` in your other instance group), you should do the same here. In the absence of GPU jobs, normal jobs could get scheduled on this node, in which case you'll have the same disk requirements as the rest of your cluster. Currently, there is no way of setting "disk" resource requests, so we have to use a convention instead.
...
If you're not seeing the node, it's possible that your resource limits (from your cloud provider) are preventing you from creating the GPU node(s). You should check your resource limits and ensure that GPU nodes are available in your region/zone.  
...
## Step 2. Creating a GPU Enabled Docker Image
...
## Step 3. Writing Your Pipeline Specification
...
## **Alternate** - Test with Local GPU

**NOTE** - This has only been tested on a linux machine.

If you want to test that your pipeline is working on a local cluster (Pachyderm in a local cluster), you can do so, but you'll need to attach the NVIDIA drivers correctly. There are two methods for this:
