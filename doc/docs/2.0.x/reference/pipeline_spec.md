...
create pipeline](../pachctl/pachctl_create_pipeline/) section.
...
Let's compare `"until_success"` and `"every_job"`:
  ...
  - Now, let's add a 4th file file4.txt to our input repo or modify the content of file2.txt for example.
      - **Case of our default `reprocess_until_success.json pipeline`**: A quick check at the [list datum on the job id](../../concepts/pipeline-concepts/datum/glob-pattern/#running-list-datum-on-a-past-job) shows 4 datums, of which 3 were skipped. (Only the changed file was processed)
...
consists of.  [Datums](../../concepts/pipeline-concepts/datum/#datum) are the *unit of parallelism* in Pachyderm.  
Per default,
...
(see [Distributed Computing](../../concepts/advanced-concepts/distributed_computing/)).
