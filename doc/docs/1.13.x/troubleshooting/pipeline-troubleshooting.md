...
1. Additionally, your stderr and pipeline logs (`pachctl log -p <pipeline name> --master` or `pachctl log -p <pipeline name> --worker`) should contain one or both of those messages:
    - number of pipelines limit exceeded:
        ```
        Pachyderm Community Edition requires an activation key to create more than 16 total pipelines (you have X).  Use the command `echo <your-activation-token> | pachctl enterprise activate` to enter your key.
    ...
    - max number of workers exceeded:
        ```
        This pipeline will only create a total of 8 workers (you specified X). Pachyderm Community Edition requires an activation key to create pipelines with constant parallelism greater than 8. Use the command `echo <your-activation-token> | pachctl enterprise activate` to enter your key.
...
Check out our [Enterprise features](../../enterprise/) for more details on our Enterprise Offer. 
