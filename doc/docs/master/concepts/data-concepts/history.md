...
Pachyderm's sub-commits and branches support a familiar Git syntax for
referencing their history. A sub-commit or branch parent can be referenced
by adding a `^` to the end of the sub-commit or branch. Similar to how
...
resolve to sub-commits on the beginning of sub-commit chains. For example,
`master.1` is the first (oldest) sub-commit on the `master` branch, `master.2`
is the second sub-commit, and so on.

Resolving ancestry syntax requires traversing chains of sub-commits
high numbers passed to `^` and low numbers passed to `.`. These operations
require traversing a large number of sub-commits which might take a long time.
