...
`putFileConcurrencyLimit` sets the maximum number of files to upload or fetch from remote sources (HTTP, blob storage) using PutFile concurrently. 

`uploadConcurrencyLimit` sets the maximum number of concurrent object storage uploads per Pachd instance.
##### Amazon
...
`customEndpoint` sets a custom s3 endpoint.

`disableSSL` disables SSL.

`id` sets the Amazon access key ID to use.

`logOptions` sets various log options in Pachyderm’s internal S3 client.  Comma-separated list containing zero or more of: 'Debug', 'Signing', 'HTTPBody', 'RequestRetries','RequestErrors', 'EventStreamBody', or 'all'
 (case-insensitive).  See 'AWS SDK for Go' docs for details.

`maxUploadParts` sets the maximum number of upload parts. Default is `10000`.

`verifySSL` performs SSL certificate verification.

`partSize` sets the part size for object storage uploads. It has to be a string due to Helm and YAML parsing integers as floats.

`region` sets the AWS region to use.

`retries` sets the number of retries for object storage requests..

`reverse` reverses object storage paths.

`secret` sets the Amazon secret access key to use.

 `timeout` sets the timeout for object storage requests.

 `token` optionally sets the Amazon token to use.

`uploadACL` sets the upload ACL for object storage uploads.

##### Google

If you're using Google Storage Buckets as your storage backend, configure it here.

`bucket` sets the object bucket to use.

`cred` is a string containing a GCP service account private key, in object (JSON or YAML) form.  A simple way to pass this on the command line is with the set-file flag, e.g.:
...
`local.hostpath` indicates the path on the host where the PFS metadata will be stored.

##### Microsoft

If you're using Microsoft Blob Storage as your storage backend, configure it here.

`container` sets the blob storage container.

`id` sets the access key ID to use.

`secret` sets the secret access key to use.

##### Minio

If you're using [MinIO](https://min.io/) as your storage backend, configure it here.

`bucket` sets the bucket to use.

`endpoint` sets the object endpoint.
...
`signature` sets the signature version to use.
...
`pgbouncer.service.type` specifies the Kubernetes type of the console service. The default is `ClusterIP`.

`pgbouncer.resources` specifies resources and limits in standard kubernetes format. It is left unset by default.
