...
We are now shipping Pachyderm with an **optional embedded proxy ([Envoy](https://www.envoyproxy.io/))** so users only need to expose one port to the Internet (for example to access `pachd` over gRPC using `pachctl`, or `console` over HTTP).
...
The architecture reference diagram below gives a quick overview of the layout of services and pods when using Envoy. In particular, it details how Pachyderm serves all external traffic on one port, then routes each call to the appropriate backend:
...
Read the following recommendations to set up your infrastructure in production or jump to our local deployment section.
...
Provision a TCP load balancer with port `80/443` forwarding to `pachyderm-proxy` service entry point.
    ...
    In general, for non-local deployments, you should load balance **all gRPC, HTTP, and S3 incoming traffic** to a TCP LB (load balanced at L4 of the OSI model) deployed in front of `pachyderm-proxy` service.
        ...
        To provision an external load balancer in your current cloud automatically (if supported), add the appropriate `annotations` in the `proxy` service of your values.yaml as follow to attach any Load Balancer configuration information to the metadata of your service:
...
Once you have your networking infrastructure is setup, 
check the [deployment page that matches your cloud provider](../../). 

## Deploy Pachyderm Locally Using Envoy
...
- To connect to your Console (Pachyderm UI), point your browser to **`localhost:80`** 
...
### NOTEBOOKS USERS: [Install Pachyderm JupyterLab Mount Extension](../../how-tos/jupyterlab-extension/#pachyderm-jupyterlab-mount-extension){target=_blank}
