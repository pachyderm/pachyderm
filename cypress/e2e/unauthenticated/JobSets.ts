describe('JobSets', () => {
  ...
  it('should allow a user to select a subset of jobsets to inspect subjobs', () => {
    ...
    cy.findAllByTestId('JobsList__row').eq(1).within(() => cy.findByLabelText('Failed datums').click());

    cy.findByTestId('Filter__FAILEDChip').should('exist');
    cy.findAllByTestId('LogRow__base').should('have.length.at.least', 19);
    cy.findAllByTestId('LogRow__base').first().parent().parent().scrollTo('bottom');
