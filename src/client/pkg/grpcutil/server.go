	...
	for _, server := range servers {
		...
		if server.PublicPortTLSAllowed {
			...
			if certPathStatErr != nil {
				log.Warnf("TLS enabled but could not stat public cert at %s: %v", certPath, certPathStatErr)
			}
			if keyPathStatErr != nil {
				log.Warnf("TLS enabled but could not stat private key at %s: %v", keyPath, keyPathStatErr)
			}
