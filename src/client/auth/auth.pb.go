...
// SessionInfo stores information associated with one OIDC authentication flow
// (i.e. a single instance of a single user logging in). Flows are stored in the
// 'oidc-authns' collection, keyed by the OIDC 'state' token (30-character
// CSPRNG-generated string). Entries are generated and inserted by
// 'GetOIDCLogin', then /authorization-code/callback retrieves an access token
// from the ID provider and sets access_token, and finally Authorize() converts
// the access token to an email address and returns a Pachyderm token
// identifying that email address's subject.
type SessionInfo struct {
	...
	// access_token contains a user's OIDC access token, which can be used to get
	// their user info.
	AccessToken string `protobuf:"bytes,2,opt,name=access_token,json=accessToken,proto3" json:"access_token,omitempty"`
	// auth_code_err contains any error encountered while exchanging an
	// authorization code for an access token (in /authorization-code/callback).
	// Storing it here allows any related calls to Authenticate() to read then
	// error and return it to pachctl, so that the CLI can print it.
	AuthCodeErr          string   `protobuf:"bytes,3,opt,name=auth_code_err,json=authCodeErr,proto3" json:"auth_code_err,omitempty"`
	XXX_NoUnkeyedLiteral struct{} `json:"-"`
	XXX_unrecognized     []byte   `json:"-"`
	XXX_sizecache        int32    `json:"-"`
}
...
func (m *SessionInfo) GetAccessToken() string {
	if m != nil {
...
func (m *SessionInfo) GetAuthCodeErr() string {
	if m != nil {
		return m.AuthCodeErr
	}
	return ""
}
...
var fileDescriptor_15ace9a5d0179ff3 = []byte{
	...
	,
}
...
func (m *SessionInfo) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.AuthCodeErr) > 0 {
		i -= len(m.AuthCodeErr)
		copy(dAtA[i:], m.AuthCodeErr)
		i = encodeVarintAuth(dAtA, i, uint64(len(m.AuthCodeErr)))
		i--
		dAtA[i] = 0x1a
	}
...
func (m *SessionInfo) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 3:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field AuthCodeErr", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.AuthCodeErr = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipAuth(dAtA[iNdEx:])
