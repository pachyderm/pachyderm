...
func (a *apiServer) GetAuthenticationCode(ctx context.Context, req *authclient.GetAuthenticationCodeRequest) (resp *authclient.GetAuthenticationCodeResponse, retErr error) {
	...
	// Get current caller's token TTL (a.k.a. their session TTL), and limit the
	// token generated by 'code' so that users can't use GetAuthenticationCode to
	...
	// ttl < 0 means that the user's session will not expire (e.g. because they
	// are a robot user)
	if ttl >= 0 {
		if ttl < 1 {
	...
	// Generation authentication code with same (or slightly shorter) expiration
	code, err := a.getAuthenticationCode(ctx, callerInfo.Subject, expiration)
	if err != nil {
		return nil, err
	}
