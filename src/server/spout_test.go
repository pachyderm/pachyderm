...
func TestSpoutPachctl(t *testing.T) {
	...
	t.Run("SpoutAuth", func(t *testing.T) {
		...
		count := 0
		require.NoError(t, c.SubscribeCommit(client.NewProjectRepo("", pipeline), "master", "", pfs.CommitState_FINISHED, func(ci *pfs.CommitInfo) error {
...
func testSpout(t *testing.T, usePachctl bool) {
	...
	t.Run("SpoutBasic", func(t *testing.T) {
		...
		jobInfos, err := c.ListProjectJob("", downstreamPipeline, nil, -1, false)
		require.NoError(t, err)
