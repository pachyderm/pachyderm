...
func TestGetFileWithEmptyCommits(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, c.PutFile(repoName, commit.Branch.Name, commit.ID, "/file", strings.NewReader("data contents"), client.WithAppendPutFile()))
	require.NoError(t, c.FinishCommit(repoName, commit.Branch.Name, commit.ID))
