...
func contextCompletion(_ *client.APIClient, _, text string, maxCompletions int64) *shell.CompletionResult {
	cfg, err := config.Read(false, false)
	if err != nil {
		log.Fatal(err)
	}
	...
	return &shell.CompletionResult{
		Suggestions: result,
		CacheFunc:   shell.CacheAll,
	}
}
