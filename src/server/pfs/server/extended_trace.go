...
func attachNewCommitsToAnyExtendedTrace(c *etcd.Client, etcdPrefix string, stm col.STM) {
	...
	golog.Printf(">>> [pfs/driver] attachCommitsToAnyExtendedTrace\n")
	defer golog.Printf(">>> [pfs/driver] (done) attachCommitsToAnyExtendedTrace\n")
	...
	if extendedTrace.Pipeline != "" {
		...
		if err := extended.TracesCol(c).ReadOnly(stm.Context()).GetByIndex(
			...
			func(key string) error {
				if key != traceID {
					fmt.Printf(">>> Deleting oldExtendedTrace:\n    %v\n", oldExtendedTrace)
	...
	golog.Printf(">>> creating extended trace with id %q + commits:\n%v\n", traceID, extendedTrace)
