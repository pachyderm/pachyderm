...
func (a *internalAPIServer) FinishCommit(ctx context.Context, request *pfsclient.FinishCommitRequest) (response *google_protobuf.Empty, retErr error) {
	...
	if err := a.driver.FinishCommit(request.Commit, request.Finished, request.Cancelled, shards); err != nil {
		return nil, err
	}
...
func (a *internalAPIServer) ListCommit(ctx context.Context, request *pfsclient.ListCommitRequest) (response *pfsclient.CommitInfos, retErr error) {
	...
	commitInfos, err := a.filteredListCommits(request.Repo, request.FromCommit, request.CommitType, request.IncludeCancelled, shards)
	if err != nil {
		return nil, err
	}
...
func (a *internalAPIServer) filteredListCommits(repos []*pfsclient.Repo, fromCommit []*pfsclient.Commit, commitType pfsclient.CommitType, includeCancelled bool, shards map[uint64]bool) ([]*pfsclient.CommitInfo, error) {
	commitInfos, err := a.driver.ListCommit(repos, fromCommit, includeCancelled, shards)
