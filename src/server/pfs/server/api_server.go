...
func (a *apiServer) GetFile(request *pfs.GetFileRequest, apiGetFileServer pfs.API_GetFileServer) (retErr error) {
	...
	file, err := a.driver.getFile(a.env.GetPachClient(apiGetFileServer.Context()), request.File, request.OffsetBytes, request.SizeBytes, false)
	if err != nil {
		return err
	}
...
func (a *apiServer) GetFiles(request *pfs.GetFileRequest, apiGetFilesServer pfs.API_GetFilesServer) (retErr error) {
	...
	sent := 0
	defer func(start time.Time) {
		a.Log(request, fmt.Sprintf("stream containing %d GetFileResponses", sent), retErr, time.Since(start))
	}(time.Now())

	// First we collect the nodes that we've gotten from getFile().
	file, err := a.driver.getFile(a.env.GetPachClient(apiGetFilesServer.Context()), request.File, request.OffsetBytes, request.SizeBytes, true)
	if err != nil {
		return err
	}
