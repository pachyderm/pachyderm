...
func (d *Driver) GetFileURL(ctx context.Context, taskService task.Service, URL string, file *pfs.File, basePathRange *pfs.PathRange) (int64, error) {
	...
	eg.Go(func() error {
		// TODO: Add cache?
		doer := taskService.NewDoer(server.URLTaskNamespace, uuid.NewWithoutDashes(), nil)
	...
	eg.Go(func() error {
		return d.Storage.Filesets.WithRenewer(ctx, server.DefaultTTL, func(ctx context.Context, renewer *fileset.Renewer) error {
			...
			createTask := func() error {
				input, err := worker.SerializeGetFileURLTask(&server.GetFileURLTask{
					URL:       URL,
...
func (d *Driver) PutFileURL(ctx context.Context, taskService task.Service, uw *fileset.UnorderedWriter, dstPath, tag string, src *pfs.AddFile_URLSource) (n int64, retErr error) {
	...
	default:
		if src.Recursive {
			return 0, d.PutFileURLRecursive(ctx, taskService, uw, dstPath, tag, src)
		}
...
func (d *Driver) PutFileURLRecursive(ctx context.Context, taskService task.Service, uw *fileset.UnorderedWriter, dst, tag string, src *pfs.AddFile_URLSource) error {
	inputChan := make(chan *anypb.Any)
	eg, ctx := errgroup.WithContext(ctx)
	doer := taskService.NewDoer(server.URLTaskNamespace, uuid.NewWithoutDashes(), nil)
	// Create tasks.
	eg.Go(func() error {
		if err := shardObjects(ctx, src.URL, func(paths []string, startOffset, endOffset int64) error {
			input, err := worker.SerializePutFileURLTask(&server.PutFileURLTask{
				Dst:         dst,
				Datum:       tag,
				URL:         src.URL,
				Paths:       paths,
	...
	eg.Go(func() error {
		...
		return task.DoOrdered(ctx, doer, inputChan, int(concurrency), func(_ int64, output *anypb.Any, _ error) error {
			result, err := worker.DeserializePutFileURLTaskResult(output)
			if err != nil {
				return err
			}
