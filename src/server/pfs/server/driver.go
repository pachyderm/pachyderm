...
type driverAPI interface {
	checkIsAuthorizedInTransaction(txnCtx *txnenv.TransactionContext, r *pfs.Repo, s auth.Scope) error
	checkIsAuthorized(pachClient *client.APIClient, r *pfs.Repo, s auth.Scope) error
	getAccessLevel(pachClient *client.APIClient, repo *pfs.Repo) (auth.Scope, error)
	fsck(pachClient *client.APIClient, fix bool, cb func(*pfs.FsckResponse) error) error

	// repo
	createRepo(txnCtx *txnenv.TransactionContext, repo *pfs.Repo, description string, update bool) error
	listRepo(pachClient *client.APIClient, includeAuth bool) (*pfs.ListRepoResponse, error)
	inspectRepo(
		txnCtx *txnenv.TransactionContext,
		repo *pfs.Repo,
		includeAuth bool,
	) (*pfs.RepoInfo, error)
	deleteRepo(txnCtx *txnenv.TransactionContext, repo *pfs.Repo, force bool) error

	// commit
	startCommit(txnCtx *txnenv.TransactionContext, ID string, parent *pfs.Commit, branch string, provenance []*pfs.CommitProvenance, description string) (*pfs.Commit, error)
	buildCommit(ctx context.Context, ID string, parent *pfs.Commit,
	...
	finishCommit(txnCtx *txnenv.TransactionContext, commit *pfs.Commit, tree *pfs.Object, empty bool, description string) (retErr error)
	finishCommitV2(txnCtx *txnenv.TransactionContext, commit *pfs.Commit, description string) error
	finishOutputCommit(txnCtx *txnenv.TransactionContext, commit *pfs.Commit, trees []*pfs.Object, datums *pfs.Object, size uint64) (retErr error)
	updateProvenanceProgress(txnCtx *txnenv.TransactionContext, success bool, ci *pfs.CommitInfo) error
	writeFinishedCommit(stm col.STM, commit *pfs.Commit, commitInfo *pfs.CommitInfo) error
	propagateCommits(stm col.STM, branches []*pfs.Branch, isNewCommit bool) error
	inspectCommit(pachClient *client.APIClient, commit *pfs.Commit, blockState pfs.CommitState) (*pfs.CommitInfo, error)
	resolveCommit(stm col.STM, userCommit *pfs.Commit) (*pfs.CommitInfo, error)
	listCommit(pachClient *client.APIClient, repo *pfs.Repo,
		to *pfs.Commit, from *pfs.Commit, number uint64, reverse bool) ([]*pfs.CommitInfo, error)
	listCommitF(pachClient *client.APIClient, repo *pfs.Repo,
		to *pfs.Commit, from *pfs.Commit, number uint64, reverse bool, f func(*pfs.CommitInfo) error) error
	subscribeCommit(pachClient *client.APIClient, repo *pfs.Repo, branch string, prov *pfs.CommitProvenance,
		from *pfs.Commit, state pfs.CommitState, f func(*pfs.CommitInfo) error) error
	flushCommit(pachClient *client.APIClient, fromCommits []*pfs.Commit, toRepos []*pfs.Repo, f func(*pfs.CommitInfo) error) error
	deleteCommit(txnCtx *txnenv.TransactionContext, userCommit *pfs.Commit) error
	resolveCommitProvenance(stm col.STM, userCommitProvenance *pfs.CommitProvenance) (*pfs.CommitProvenance, error)

	// branch
	createBranch(txnCtx *txnenv.TransactionContext, branch *pfs.Branch, commit *pfs.Commit, provenance []*pfs.Branch) error
	inspectBranch(txnCtx *txnenv.TransactionContext, branch *pfs.Branch) (*pfs.BranchInfo, error)
	listBranch(pachClient *client.APIClient, repo *pfs.Repo, reverse bool) ([]*pfs.BranchInfo, error)
	deleteBranch(txnCtx *txnenv.TransactionContext, branch *pfs.Branch, force bool) error
	scratchCommitPrefix(commit *pfs.Commit) string
	checkFilePath(path string) error
	scratchFilePrefix(file *pfs.File) (string, error)
	putFile(pachClient *client.APIClient, file *pfs.File, delimiter pfs.Delimiter,
	...
	putFiles(pachClient *client.APIClient, s *putFileServer) error
	copyFile(pachClient *client.APIClient, src *pfs.File, dst *pfs.File, overwrite bool) (retErr error)
	getTreeForCommit(txnCtx *txnenv.TransactionContext, commit *pfs.Commit) (hashtree.HashTree, error)
	getTree(pachClient *client.APIClient, commitInfo *pfs.CommitInfo, path string) (rs []io.ReadCloser, retErr error)
	getTrees(pachClient *client.APIClient, commitInfo *pfs.CommitInfo, pattern string) (rs []io.ReadCloser, retErr error)
	getTreeForFile(pachClient *client.APIClient, file *pfs.File) (hashtree.HashTree, error)
	getTreeForOpenCommit(pachClient *client.APIClient, file *pfs.File, parentTree hashtree.HashTree) (result hashtree.HashTree, retErr error)
	getFile(pachClient *client.APIClient, file *pfs.File, offset int64, size int64) (r io.Reader, retErr error)
	inspectFile(pachClient *client.APIClient, file *pfs.File) (fi *pfs.FileInfo, retErr error)
	listFile(pachClient *client.APIClient, file *pfs.File, full bool, history int64, f func(*pfs.FileInfo) error) (retErr error)
	fileHistory(pachClient *client.APIClient, file *pfs.File, history int64, f func(*pfs.FileInfo) error) error
	walkFile(pachClient *client.APIClient, file *pfs.File, f func(*pfs.FileInfo) error) (retErr error)
	globFile(pachClient *client.APIClient, commit *pfs.Commit, pattern string, f func(*pfs.FileInfo) error) (retErr error)
	diffFile(pachClient *client.APIClient, newFile *pfs.File, oldFile *pfs.File, shallow bool) ([]*pfs.FileInfo, []*pfs.FileInfo, error)
	deleteFile(pachClient *client.APIClient, file *pfs.File) error
	deleteAll(txnCtx *txnenv.TransactionContext) error
	applyWrite(key string, records *pfs.PutFileRecords, tree hashtree.HashTree) error
	addBranchProvenance(branchInfo *pfs.BranchInfo, provBranch *pfs.Branch, stm col.STM) error
	appendSubvenance(commitInfo *pfs.CommitInfo, subvCommitInfo *pfs.CommitInfo)
	forEachPutFile(pachClient *client.APIClient, server pfs.API_PutFileServer, f func(*pfs.PutFileRequest, io.Reader) error) (bool, string, string, error)

	withFileSet(ctx context.Context, repo, commit string, f func(*fileset.FileSet) error) (retErr error)
	getTar(ctx context.Context, repo, commit, glob string, w io.Writer) error
	listFileV2(pachClient *client.APIClient, file *pfs.File, full bool, history int64, f func(*pfs.FileInfoV2) error) error
	getTarConditional(ctx context.Context, repo, commit, glob string, f func(*FileReader) error) error
}
