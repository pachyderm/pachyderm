...
func (d *driver) runTransaction(pachClient *client.APIClient, requests []Operation) ([][]byte, error) {
	...
	_, err := col.NewSTM(pachClient.Ctx(), d.etcdClient, func(stm col.STM) error {
		...
		// re-initialize the results array every time
		results = [][]byte{}
...
func (d *driver) makeCommit(
	...
	if branch != "" {
		if err := d.propagateCommit(stm, client.NewBranch(newCommit.Repo.Name, branch)); err != nil {
			return nil, err
		}
	}
...
func (req *CreateBranchRequest) runInTransaction(d *driver, pachClient *client.APIClient, stm col.STM) ([]byte, error) {
	...
	if req.Head != nil {
		...
		if !sameTarget && req.Provenance != nil {
			return nil, fmt.Errorf(
				"cannot point branch \"%s\" at target commit \"%s/%s\" without clearing its provenance",
	...
	if req.Head != nil {
		...
		if err != nil {
			...
			// branchInfo.Head must also be nil
			if !isNoHeadErr(err) {
				return nil, fmt.Errorf("unable to inspect %s/%s: %v", err, req.Head.Repo.Name, req.Head.ID)
			}
			req.Head = nil
		}
	}
	...
	// creating a new HEAD commit if 'branch's provenance was changed and its
	// current HEAD commit has old provenance
	return nil, d.propagateCommit(stm, req.Branch)
}
...
func (d *driver) groupPutFileResult(
	...
	if oneOff {
		...
		submap[branch] = append(submap[branch], result)
	} else {
...
func (d *driver) putFiles(pachClient *client.APIClient, s *putFileServer) error {
	...
	// For any files written to open commits, update the blob store and continue
	for _, result := range openRecords {
		if err := d.upsertPutFileRecords(pachClient, result.file, result.records); err != nil {
			return err
		}
	}
		...
		_, err := col.NewSTM(pachClient.Ctx(), d.etcdClient, func(stm col.STM) error {
			for repo, resultsByBranch := range oneOffRecords {
				for branch, results := range resultsByBranch {
					...
					_, err := d.makeCommit(pachClient, stm, "", client.NewCommit(repo, ""), branch, nil, nil, paths, records, "")
					if err != nil {
						return err
					}
				}
