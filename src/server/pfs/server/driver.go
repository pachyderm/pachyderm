...
func newDriver(env serviceenv.ServiceEnv, txnEnv *txnenv.TransactionEnv, etcdPrefix string) (*driver, error) {
	...
	chunkStorageOpts, err := storage.ChunkOptions(env.Config())
	if err != nil {
		return nil, err
	}
	...
	d.storage = fileset.NewStorage(fileset.NewPostgresStore(env.GetDBClient()), tracker, chunkStorage, storage.FilesetOptions(env.Config())...)
...
func (d *driver) deleteRepo(txnCtx *txncontext.TransactionContext, repo *pfs.Repo, force bool) error {
	...
	if !force {
		if _, err := d.env.PpsServer().InspectPipelineInTransaction(txnCtx, repo.Name); err == nil {
