...
// caller to start the returned server and close `errLogWriter`. It's possible
// for the caller to gracefully shutdown the server if desired; see the `http`
// package for details.
//
...
func Server(pc *client.APIClient, port uint16, errLogWriter io.Writer) *http.Server {
	...
	multipartHandler, err := newMultipartHandler(pc)
	if err != nil {
		panic(err)
	}
