...
func (a *apiServer) Extract(request *admin.ExtractRequest, extractServer admin.API_ExtractServer) (retErr error) {
	...
	if !request.NoAuth {
		...
		if err := writeOp(&admin.Op{Op1_13: &admin.Op1_13{CheckAuthToken: &admin.CheckAuthToken{}}}); err != nil {
			return err
		}
	}
	...
	if !request.NoObjects {
		...
		if err := pachClient.ListObject(func(oi *pfs.ObjectInfo) error {
			return writeOp(&admin.Op{Op1_13: &admin.Op1_13{CreateObject: &pfs.CreateObjectRequest{
				Object:   oi.Object,
		...
		if err := pachClient.ListTag(func(resp *pfs.ListTagsResponse) error {
			return writeOp(&admin.Op{Op1_13: &admin.Op1_13{
	...
	if !request.NoRepos {
		...
		for i := range ris {
			ri := ris[len(ris)-1-i]
			if err := writeOp(&admin.Op{Op1_13: &admin.Op1_13{
		...
		if err := pachClient.ListCommitF("", "", "", 0, true, func(ci *pfs.CommitInfo) error {
			...
			return writeOp(&admin.Op{Op1_13: &admin.Op1_13{Commit: &pfs.BuildCommitRequest{
				Origin:     ci.Origin,
	...
	if !request.NoPipelines {
		...
		for _, pi := range pis {
			...
			if err := writeOp(&admin.Op{Op1_13: &admin.Op1_13{Pipeline: cPR}}); err != nil {
				return err
			}
			if err := pachClient.ListJobF(pi.Pipeline.Name, nil, nil, -1, false, func(ji *pps.JobInfo) error {
				return writeOp(&admin.Op{Op1_13: &admin.Op1_13{Job: &pps.CreateJobRequest{
	...
	if !request.NoEnterprise {
		...
		if state.State == enterprise.State_ACTIVE {
			if err := writeOp(&admin.Op{Op1_13: &admin.Op1_13{ActivateEnterprise: &enterprise.ActivateRequest{ActivationCode: state.ActivationCode}}}); err != nil {
				return err
			}
		} else {
	...
	if !request.NoAuth {
		...
		if err := writeOp(&admin.Op{Op1_13: &admin.Op1_13{ActivateAuth: &auth.ActivateRequest{RootToken: ""}}}); err != nil {
			return err
		}
		...
		for _, t := range robotTokens.Tokens {
			if err := writeOp(&admin.Op{Op1_13: &admin.Op1_13{RestoreAuthToken: &auth.RestoreAuthTokenRequest{Token: t}}}); err != nil {
		...
		for principal, roles := range admins.Bindings {
			if err := writeOp(&admin.Op{Op1_13: &admin.Op1_13{SetClusterRoleBinding: &auth.ModifyClusterRoleBindingRequest{
				Principal: principal,
				Roles:     roles,
			}}}); err != nil {
				return err
		...
		if err := writeOp(&admin.Op{Op1_13: &admin.Op1_13{SetAuthConfig: &auth.SetConfigurationRequest{Configuration: config.Configuration}}}); err != nil {
			return err
		}
		...
		if !request.NoRepos {
			for _, ri := range ris {
				...
				if err := writeOp(&admin.Op{Op1_13: &admin.Op1_13{SetAcl: &auth.SetACLRequest{
					Repo:    ri.Repo.Name,
					Entries: acl.Entries,
				}}}); err != nil {
					return err
...
func (a *apiServer) ExtractPipeline(ctx context.Context, request *admin.ExtractPipelineRequest) (response *admin.Op, retErr error) {
	...
	return &admin.Op{Op1_13: &admin.Op1_13{Pipeline: ppsutil.PipelineReqFromInfo(pi)}}, nil
}
...
func (r *restoreCtx) validateAndApplyOp(op *admin.Op) error {
	...
	case v1_13:
		return r.applyOp1_13(op.Op1_13)
	default:
...
func (r *restoreCtx) applyOp1_12(op *admin.Op1_12) error {
	...
	default:
		...
		if err := r.applyOp1_13(newOp); err != nil {
			return err
		}
		return nil
	}
...
func (r *restoreCtx) applyOp1_13(op *admin.Op1_13) error {
	switch {
	case op.Object != nil:
		extractReader := &extractObjectReader{
			adminAPIRestoreServer: r.restoreServer,
...
func (r *restoreCtx) applyOp(op *admin.Op1_13) error {
	c := r.pachClient
...
func (w extractBlockWriter) Write(p []byte) (int, error) {
	...
	for i := 0; i*(chunkSize) < len(p); i++ {
		...
		if err := w.f(&admin.Op{Op1_13: &admin.Op1_13{Block: &pfs.PutBlockRequest{Block: w.block, Value: value}}}); err != nil {
			return n, err
		}
...
func (w extractBlockWriter) Close() error {
	return w.f(&admin.Op{Op1_13: &admin.Op1_13{Block: &pfs.PutBlockRequest{Block: w.block}}})
}
...
func (r *extractBlockReader) Read(p []byte) (int, error) {
	...
	for len(p) > r.buf.Len() && !r.eof {
		...
		if r.version == v1_7 {
			...
			if op.Op1_13.Block == nil {
				return 0, errors.Errorf("expected a block, but got: %v", op)
			}
			value = op.Op1_13.Block.Value
		}
