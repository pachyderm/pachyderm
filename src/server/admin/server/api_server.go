...
func (r *restoreCtx) startFromURL(reqURL string) error {
	...
	if err != nil {
		return fmt.Errorf("error parsing url %v: %v", reqURL, err)
	}
	if url.Object == "" {
		return fmt.Errorf("URL must be <svc>://<bucket>/<object> (no object in %s)", reqURL)
	}
	objClient, err := obj.NewClientFromURLAndSecret(url, false)
	if err != nil {
		return err
...
func (r *restoreCtx) validateAndApplyOp(op *admin.Op) error {
	...
	default:
		return fmt.Errorf("unrecognized stream version: %s", r.streamVersion)
	}
}
