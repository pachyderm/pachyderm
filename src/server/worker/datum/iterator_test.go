...
func TestIterators(t *testing.T) {
	...
	t.Run("Join", func(t *testing.T) {
		join1, err := datum.NewIterator(c, in10)
		require.NoError(t, err)
		validateDI(t, join1,
			"/foo11/foo11",
			"/foo12/foo21",
	...
	t.Run("OuterJoin", func(t *testing.T) {
		join1, err := datum.NewIterator(c, in13)
		require.NoError(t, err)
		validateDI(t, join1,
			"/foo10",
			"/foo11/foo11",
	...
	t.Run("GroupSingle", func(t *testing.T) {
		group1, err := datum.NewIterator(c, in15)
		require.NoError(t, err)
		validateDI(t, group1,
	...
	t.Run("GroupDoubles", func(t *testing.T) {
		group2, err := datum.NewIterator(c, in18)
		require.NoError(t, err)
		validateDI(t, group2,
	...
	t.Run("GroupJoin", func(t *testing.T) {
		groupJoin1, err := datum.NewIterator(c, in22)
		require.NoError(t, err)
	...
	t.Run("UnionGroup", func(t *testing.T) {
		unionGroup1, err := datum.NewIterator(c, in26)
		require.NoError(t, err)
	...
	t.Run("PlainS3", func(t *testing.T) {
		di, err := datum.NewIterator(c, in27)
		require.NoError(t, err)
		validateDI(t, di, "/")
	...
	t.Run("S3MixedCross", func(t *testing.T) {
		di, err := datum.NewIterator(c, in28)
		require.NoError(t, err)
		validateDI(t, di,
			"/foo11/foo12/", "/foo11/foo2/", "/foo11/foo22/", "/foo11/foo32/", "/foo11/foo42/",
			"/foo21/foo12/", "/foo21/foo2/", "/foo21/foo22/", "/foo21/foo32/", "/foo21/foo42/",
	...
	t.Run("S3OnlyCrossUnionJoin", func(t *testing.T) {
		di, err := datum.NewIterator(c, in29)
		require.NoError(t, err)
		validateDI(t, di, "///")
	})
}
...
//	input[0].Pfs.Glob = "/(*)"
//	input[1].Pfs.Glob = "/(*)"
//	itr, err := datum.NewIterator(c, client.NewJoinInput(input...))
//	require.NoError(t, err)
//	validateDI(t, itr,
//		"/foo-0/foo-0",
//		"/foo-1/foo-1",
