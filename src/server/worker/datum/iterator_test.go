...
func TestIterators(t *testing.T) {
	...
	t.Run("S3MixedCrossUnionJoin", func(t *testing.T) {
		...
		require.NoError(t, err)
		s3UnionItr, err := NewIterator(c, in13)
		require.NoError(t, err)
		s3JoinItr, err := NewIterator(c, in14)
		require.NoError(t, err)
		require.NoError(t, err)
	...
	t.Run("S3OnlyCrossUnionJoin", func(t *testing.T) {
		s3CrossItr, err := NewIterator(c, in15)
		require.NoError(t, err)
		s3UnionItr, err := NewIterator(c, in16)
		require.NoError(t, err)
		s3JoinItr, err := NewIterator(c, in17)
		require.NoError(t, err)
		require.NoError(t, err)
		validateDI(t, s3CrossItr, "")
	...
	t.Run("NestedS3", func(t *testing.T) {
		s3InnerMixedCrossItr, err := NewIterator(c, in18)
		require.NoError(t, err)
		s3InnerS3OnlyCrossItr, err := NewIterator(c, in19)
		require.NoError(t, err)
