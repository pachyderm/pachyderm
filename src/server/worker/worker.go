	...
	backoff.RetryUntilCancel(ctx, func() error {
		...
		if env.Config().PPSWorkerPreprocessing {
			...
			eg.Go(func() error {
				...
				return transform.PreprocessingWorker(ctx, driver.PachClient().PfsAPIClient, env.GetTaskService(etcdPrefix), driver.PipelineInfo(), w.driver.PachClient().PjsAPIClient)
			})
		}
