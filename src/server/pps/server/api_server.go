...
func (a *apiServer) getJobDetails(ctx context.Context, jobInfo *pps.JobInfo) error {
	...
	if jobInfo.State == pps.JobState_JOB_RUNNING {
		workerStatus, err := workerserver.Status(ctx, jobInfo.Job.Pipeline.Project.GetName(), jobInfo.Job.Pipeline.Name, jobInfo.PipelineVersion, a.env.EtcdClient, a.etcdPrefix, a.workerGrpcPort)
		if err != nil {
...
func (a *apiServer) validatePipelineRequest(request *pps.CreatePipelineRequest) error {
	...
	if err := pps.ValidatePipelineName(request.Pipeline.Name); err != nil {
...
func (a *apiServer) propagateJobs(txnCtx *txncontext.TransactionContext) error {
	...
	for _, commitInfo := range commitInfos {
		...
		var (
			pipeline = &pps.Pipeline{
				Project: commitInfo.Commit.Branch.Repo.Project,
				Name:    commitInfo.Commit.Branch.Repo.Name,
			}
			pipelineInfo *pps.PipelineInfo
		)

		if pipelineInfo, err = a.InspectPipelineInTransaction(txnCtx, pipeline); err != nil {
			if col.IsErrNotFound(err) {
				continue
			}
			return err
...
func (a *apiServer) rcPods(ctx context.Context, projectName, pipelineName string, pipelineVersion uint64) ([]v1.Pod, error) {
	// FIXME: should this use Selector?
	labels := map[string]string{
		appLabel:             "pipeline",
