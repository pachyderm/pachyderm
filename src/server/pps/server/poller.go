...
func (m *ppsMaster) pollPipelines(ctx context.Context) {
	...
	if err := backoff.RetryUntilCancel(ctx, backoff.MustLoop(func() error {
			...
			if rcs != nil {
				for _, rc := range rcs.Items {
					...
					pipeline := &pps.Pipeline{
						Project: &pfs.Project{Name: rc.Labels[pipelineProjectLabel]},
						Name:    pipelineName,
					}
		...
		if err != nil {
			return errors.Errorf("invalid pipeline key %q in dbPipelines", pKey)
		}
		select {
		...
		for {
			...
			case event, ok := <-watch:
				...
				crashPipeline := func(reason string) error {
					...
					pipelineVersion, versionErr := strconv.Atoi(pod.ObjectMeta.Annotations[pipelineVersionAnnotation])
					if versionErr != nil {
