	...
	createJob := &cobra.Command{
		...
		Run: cmdutil.RunFixedArgs(0, func(args []string) (retErr error) {
			...
			if len(request.Inputs) != 0 {
				fmt.Printf("WARNING: field `inputs` will be deprecated in v1.6. Both formats are valid for v1.4.6 to 1.5.x. See docs for the new input format: http://pachyderm.readthedocs.io/en/latest/reference/pipeline_spec.html \n")
			}
			if pushImages {
