...
func TestUpgradeOpenCVWithAuth(t *testing.T) {
	...
	upgradeTest(t, context.Background(), true /* parallelOK */, fromVersions,
		func(t *testing.T, c *client.APIClient, from string) { /* preUpgrade */
			...
			commitInfo, err := c.InspectProjectCommit(pfs.DefaultProjectName, montage(from), "master", "")
			require.NoError(t, err)
			...
			for _, info := range commitInfos {
				if proto.Equal(info.Commit.Repo, client.NewProjectRepo(pfs.DefaultProjectName, montage(from))) {
