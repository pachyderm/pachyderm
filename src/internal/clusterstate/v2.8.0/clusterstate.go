...
func Migrate(state migrations.State) migrations.State {
	return state.
		...
		Apply("Migrate collections.branches to pfs.branches", func(ctx context.Context, env migrations.Env) error {
			if err := migrateBranches(ctx, env.Tx); err != nil {
				return errors.Wrap(err, "migrating branches")
			}
			return nil
		}, migrations.Squash).
