...
func TestPgDumpEnvironment(t *testing.T) {
	old := libpqRlocation
	defer func() {
		libpqRlocation = old
	}()
	testData := []struct {
		...
		},
		{
			...
			want:    []string{"FOO=bar", "BAR=baz", "LD_LIBRARY_PATH=/lib"},
		},
		{
			name:      "create new variable",
			environ:   []string{"FOO=bar", "BAR=baz"},
			rlocation: "/foo/libpq.so.5.17",
			want:      []string{"FOO=bar", "BAR=baz", "LD_LIBRARY_PATH=/foo"},
		},
		{
			name:      "prepend to existing",
			environ:   []string{"FOO=bar", "BAR=baz", "LD_LIBRARY_PATH=/lib:/usr/lib"},
			rlocation: "/foo/libpq.so.5.17",
			want:      []string{"FOO=bar", "BAR=baz", "LD_LIBRARY_PATH=/foo:/lib:/usr/lib"},
		},
	}
	for _, test := range testData {
		t.Run(test.name, func(t *testing.T) {
			libpqRlocation = test.rlocation
			got := pgDumpEnviron(test.environ)
			if diff := cmp.Diff(test.want, got, cmpopts.EquateEmpty()); diff != "" {
				t.Errorf("pgDumpEnviron() (-want +got):\n%s", diff)
			}
		})
	}
}
