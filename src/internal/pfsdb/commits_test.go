...
func TestCreateCommitWithParent(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			got, err := pfsdb.GetCommit(ctx, tx, commitID)
			require.NoError(t, err, "should be able to get commit")
			require.Equal(t, got.Parent.Pb().Key(), parentInfo.Commit.Key())
		})
	})
}
...
func TestCreateCommitWithRelatives(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			require.NoError(t, err, "should be able to get commit")
			require.Equal(t, got.Parent.Pb().Key(), parentInfo.Commit.Key())
			require.Equal(t, got.Children[0].Key, childInfo.Commit.Key())
			require.Equal(t, got.Children[1].Key, childInfo2.Commit.Key())
		})
	})
}
...
func TestCreateCommit(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			_, err := pfsdb.CreateCommit(ctx, tx, commitInfo)
			require.NoError(t, err, "should be able to create commit")
			require.NoError(t, pfsdb.UpdateCommitBranch(ctx, tx, 1, createBranch(ctx, t, tx, commitInfo.Commit)))
			got, err := pfsdb.GetCommit(ctx, tx, 1)
			require.NoError(t, err)
			commitsMatch(t, commitInfo, got.PbInfo())
			commitInfo = testCommit(ctx, t, tx, testRepoName)
...
func TestGetCommitRepoMissing(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			_, err := pfsdb.GetCommitByKey(ctx, tx, commitInfo.Commit)
...
func TestGetCommitInfo(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			commitID, err := pfsdb.CreateCommit(ctx, tx, commitInfo)
			require.NoError(t, err, "should be able to create commit")
			require.NoError(t, pfsdb.UpdateCommitBranch(ctx, tx, commitID, createBranch(ctx, t, tx, commitInfo.Commit))) // add branch fields once they exist
			got, err := pfsdb.GetCommit(ctx, tx, commitID)
			...
			got, err = pfsdb.GetCommitByKey(ctx, tx, commitInfo.Commit)
...
func TestDeleteCommitWithNoRelatives(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			require.NoError(t, err, "should be able to create commit")
			err = pfsdb.DeleteCommit(ctx, tx, id)
			require.NoError(t, err, "should be able to delete commit")
			_, err = pfsdb.GetCommit(ctx, tx, id)
...
func TestDeleteCommitWithParent(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			parentInfo := testCommit(ctx, t, tx, testRepoName)
			parentID, err := pfsdb.CreateCommit(ctx, tx, parentInfo)
			...
			parent, err := pfsdb.GetCommit(ctx, tx, parentID)
			require.NoError(t, err, "should be able to get parent commit row id")
			require.Equal(t, len(parent.Children), 1, "there should only be 1 child")
			require.Equal(t, parent.Children[0].ID, id, "commit should be parent's child")
			// delete commit
			err = pfsdb.DeleteCommit(ctx, tx, id)
			require.NoError(t, err, "should be able to delete commit")
			_, err = pfsdb.GetCommit(ctx, tx, id)
...
func TestDeleteCommitWithChildren(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			// setup parent and commit
			commitInfo := testCommit(ctx, t, tx, testRepoName)
			parentID, err := pfsdb.CreateCommit(ctx, tx, commitInfo)
			require.NoError(t, err, "should be able to create commit")
			childInfo := testCommit(ctx, t, tx, testRepoName)
			childInfo2 := testCommit(ctx, t, tx, testRepoName)
			...
			child, err := pfsdb.GetCommitByKey(ctx, tx, childInfo.Commit)
			require.NoError(t, err, "should be able to get child commit row id")
			child2, err := pfsdb.GetCommitByKey(ctx, tx, childInfo2.Commit)
			require.NoError(t, err, "should be able to get child commit 2 row id")
			require.Equal(t, child.Parent.ID, child2.Parent.ID, "child 1 and child 2 should have same parent")
			require.Equal(t, child.Parent.ID, parentID, "parent id should be commit id")
			// delete commit
			err = pfsdb.DeleteCommit(ctx, tx, parentID)
			require.NoError(t, err, "should be able to delete commit")
			_, err = pfsdb.GetCommit(ctx, tx, parentID)
			require.YesError(t, err, "commit should not exist")
			require.True(t, errors.As(err, &pfsdb.CommitNotFoundError{}))
			// confirm children has no parent.
			_, err = pfsdb.GetCommitParent(ctx, tx, child.ID)
			require.YesError(t, err, "parent of child 1 should not exist")
			require.True(t, errors.As(err, &pfsdb.ParentCommitNotFoundError{}))
			_, err = pfsdb.GetCommitParent(ctx, tx, child2.ID)
...
func TestDeleteCommitWithRelatives(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			// delete commit
			err = pfsdb.DeleteCommit(ctx, tx, commitID)
			require.NoError(t, err, "should be able to delete commit")
...
func TestUpsertCommit_WithMetadata(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			require.NoError(t, pfsdb.UpdateCommitBranch(ctx, tx, 1, createBranch(ctx, t, tx, commitInfo.Commit))) // do an update to add the branch fields.
			got, err := pfsdb.GetCommitByKey(ctx, tx, commitInfo.Commit)
			require.NoError(t, err, "should be able to get a commit by key")
			commitsMatch(t, commitInfo, got.PbInfo())
...
func checkOutput(ctx context.Context, t *testing.T, iter stream.Iterator[pfsdb.Commit], expectedInfos []*pfs.CommitInfo) {
	i := 0
	require.NoError(t, stream.ForEach[pfsdb.Commit](ctx, iter, func(commit pfsdb.Commit) error {
		got := commit
		commitsMatch(t, expectedInfos[i], got.PbInfo())
		i++
		return nil
	}))
...
func TestListCommit(t *testing.T) {
	...
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			var prevCommit *pfsdb.Commit
			for i := 0; i < size; i++ {
				commitInfo := testCommit(ctx, t, tx, testRepoName)
				commitInfo.Commit.Branch = nil // other tests will handle correct creation of branches.
				if prevCommit != nil {
					commitInfo.ParentCommit = prevCommit.Pb()
				}
				commitID, err := pfsdb.CreateCommit(ctx, tx, commitInfo)
				require.NoError(t, err, "should be able to create commit")
				...
				require.NoError(t, err, "should be able to create commit")
				if prevCommit != nil {
					require.NoError(t, pfsdb.CreateCommitAncestries(ctx, tx, prevCommit.ID, []pfsdb.CommitID{commitID}))
	...
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			for i := 0; i < size; i++ {
				...
				_, err := pfsdb.CreateCommit(ctx, tx, commitInfo)
				require.NoError(t, err, "should be able to create commit")
				require.NoError(t, pfsdb.UpdateCommitBranch(ctx, tx, pfsdb.CommitID(i+1), createBranch(ctx, t, tx, commitInfo.Commit)))
			}
		})
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
		...
		for i := 0; i < 10; i++ {
			withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
				for j := 0; j < 3; j++ {
					...
					commitID, err := pfsdb.CreateCommit(ctx, tx, commitInfo)
					require.NoError(t, err, "should be able to create commit")
					require.NoError(t, pfsdb.UpdateCommitBranch(ctx, tx, commitID, createBranch(ctx, t, tx, commitInfo.Commit)))
				}
			})
		}
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
...
func testPickCommitByID(t *testing.T) {
	...
	withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
		...
		globalIDPicker.Picker.(*pfs.CommitPicker_Id).Id.Id = commit.SetID
...
func testPickCommitByBranchRoot(t *testing.T) {
	...
	db := newTestDB(t, ctx)
	depth := 1200
	expectedID := pfsdb.CommitID(offset + 1) // ID starts at 1.
	withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
...
func testPickCommitByAncestor(t *testing.T) {
	...
	expectedID := headId - pfsdb.CommitID(offset)
	withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
		makeCommitTree(ctx, t, depth, db)
