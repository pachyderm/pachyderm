...
func TestCreateCommitWithMissingParent(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withFailedTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			require.True(t, errors.As(err, &pfsdb.ErrParentCommitNotFound{ChildRowID: 1}))
		})
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			commitInfo := testCommit(ctx, t, tx, testRepoName)
			parentInfo := testCommit(ctx, t, tx, testRepoName)
...
func TestCreateCommitWithMissingChild(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withFailedTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			require.True(t, errors.As(err, &pfsdb.ErrChildCommitNotFound{ParentRowID: 1}))
		})
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			parentInfo := testCommit(ctx, t, tx, testRepoName)
			commitInfo := testCommit(ctx, t, tx, testRepoName)
...
func TestCreateCommit(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			require.True(t, errors.As(err, &pfsdb.ErrCommitMissingInfo{Field: "Repo"}))
			commitInfo = testCommit(ctx, t, tx, testRepoName)
			commitInfo.Origin = nil
			_, err = pfsdb.CreateCommit(ctx, tx, commitInfo)
			require.YesError(t, err, "should not be able to create commit when origin is nil")
			require.True(t, errors.As(err, &pfsdb.ErrCommitMissingInfo{Field: "Origin"}))
			commitInfo = testCommit(ctx, t, tx, testRepoName)
			commitInfo.Commit = nil
			_, err = pfsdb.CreateCommit(ctx, tx, commitInfo)
			require.YesError(t, err, "should not be able to create commit when commit is nil")
			require.True(t, errors.As(err, &pfsdb.ErrCommitMissingInfo{Field: "Commit"}))
		})
		withFailedTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			commitInfo := testCommit(ctx, t, tx, testRepoName)
			_, err := pfsdb.CreateCommit(ctx, tx, commitInfo)
			...
			require.True(t, errors.As(err, &pfsdb.ErrCommitAlreadyExists{CommitID: pfsdb.CommitKey(commitInfo.Commit)}))
		})
	})
}
...
func TestGetCommitRepoMissing(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			require.True(t, errors.As(err, &pfsdb.ErrRepoNotFound{Project: repoInfo.Repo.Project.Name, Name: repoInfo.Repo.Name, Type: testRepoType}))
		})
	})
}
...
func TestGetCommit(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			require.True(t, errors.As(err, &pfsdb.ErrCommitNotFound{RowID: 3}))
...
func TestDeleteCommitWithNoRelatives(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			require.True(t, errors.As(err, &pfsdb.ErrCommitNotFound{CommitID: pfsdb.CommitKey(commitInfo.Commit)}))
			err = pfsdb.DeleteCommit(ctx, tx, commitInfo.Commit)
...
func TestDeleteCommitWithChildren(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			require.True(t, errors.As(err, &pfsdb.ErrParentCommitNotFound{ChildRowID: childID}))
			_, err = pfsdb.GetCommitParent(ctx, tx, childID2)
			require.YesError(t, err, "parent of child 2 should not exist")
			require.True(t, errors.As(err, &pfsdb.ErrParentCommitNotFound{ChildRowID: childID2}))
		})
	})
}
...
func TestUpdateProjectMissing(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			require.True(t, errors.As(err, &pfsdb.ErrProjectNotFound{Name: "doesNotExist"}))
		})
	})
}
...
func TestUpdateCommitRemoveParent(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			require.True(t, errors.As(err, &pfsdb.ErrParentCommitNotFound{ChildRowID: id}))
		})
	})
}
...
func TestUpdateCommitRemoveChild(t *testing.T) {
	withDB(t, func(ctx context.Context, t *testing.T, db *pachsql.DB) {
		withTx(t, ctx, db, func(ctx context.Context, tx *pachsql.Tx) {
			...
			require.True(t, errors.As(err, &pfsdb.ErrChildCommitNotFound{ParentRowID: id}))
		})
	})
}
