	...
	t.Run("valid/walk/all", func(t *testing.T) {
		...
		withDependencies(t, func(d dependencies) {
			...
			t.Run("level_order", func(t *testing.T) {
				jobs, err := pjsdb.WalkJob(d.ctx, d.tx, 1, pjsdb.LevelOrder)
				require.NoError(t, err)
			...
			t.Run("pre_order", func(t *testing.T) {
				jobs, err := pjsdb.WalkJob(d.ctx, d.tx, 1, pjsdb.PreOrder)
				require.NoError(t, err)
				require.Equal(t, numJobs, len(jobs))
				ids := jobIDs(jobs)
			...
			t.Run("post_order", func(t *testing.T) {
				jobs, err := pjsdb.WalkJob(d.ctx, d.tx, 1, pjsdb.MirroredPostOrder)
				require.NoError(t, err)
				require.Equal(t, numJobs, len(jobs))
				ids := jobIDs(jobs)
	...
	t.Run("valid/walk/subset", func(t *testing.T) {
		...
		*/
		withDependencies(t, func(d dependencies) {
			fullBinaryJobTree(t, d, maxDepth)
			t.Run("level_order", func(t *testing.T) {
				jobs, err := pjsdb.WalkJob(d.ctx, d.tx, 3, pjsdb.LevelOrder)
				require.NoError(t, err)
			...
			t.Run("pre_order", func(t *testing.T) {
				jobs, err := pjsdb.WalkJob(d.ctx, d.tx, 3, pjsdb.PreOrder)
				require.NoError(t, err)
				require.Equal(t, 3, len(jobs))
				ids := jobIDs(jobs)
				require.NoDiff(t, []pjsdb.JobID{3, 5, 6}, ids, nil)
			...
			t.Run("mirrored_post_order", func(t *testing.T) {
				jobs, err := pjsdb.WalkJob(d.ctx, d.tx, 3, pjsdb.MirroredPostOrder)
				require.NoError(t, err)
				require.Equal(t, 3, len(jobs))
				ids := jobIDs(jobs)
