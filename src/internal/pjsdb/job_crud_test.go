...
func createRootJob(t *testing.T, d dependencies) pjsdb.JobID {
	filesetID := mockFileset(t, d, "/", "")
	req, err := pjsdb.CreateJobRequest{
		Spec:  filesetID.HexString(),
		Input: "",
	}.Sanitize(d.ctx, d.tx, d.s)
...
func createJob(t *testing.T, d dependencies, parent pjsdb.JobID) (pjsdb.JobID, error) {
	req, err := makeReq(t, d, parent, nil).Sanitize(d.ctx, d.tx, d.s)
	if err != nil {
		return 0, err
	}
	id, err := pjsdb.CreateJob(d.ctx, d.tx, req)
...
func TestListJobTxByFilter(t *testing.T) {
	t.Run("valid/spec", func(t *testing.T) {
		withDependencies(t, func(d dependencies) {
			...
			for i := 0; i < 5; i++ {
				included, err := pjsdb.GetJob(d.ctx, d.tx, createJobWithFilesets(t, d, 0, targetFs, nil))
				require.NoError(t, err)
