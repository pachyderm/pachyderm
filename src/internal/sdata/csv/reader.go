...
parseField:
	for {
		...
		if len(line) == 0 || line[0] != '"' {
			...
			// Additional logic here to decided whether the field is a nil.
			if len(field) == 0 {
				r.nilIndexes = append(r.nilIndexes, true)
			} else {
				r.nilIndexes = append(r.nilIndexes, false)
			}
			if i >= 0 {
	...
	for i, idx := range r.fieldIndexes {
		...
		dst[i] = &v
		preIdx = idx
		// Additional logic to actually set nil values.
		if r.nilIndexes[i] {
			dst[i] = nil
		}
	}
