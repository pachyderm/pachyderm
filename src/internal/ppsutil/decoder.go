...
func NewPipelineManifestReaderFromJsonnet(filename string, templateBytes []byte, params map[string]string) (*PipelineManifestReader, error) {
	// as jsonnet is executable (not just text subsitution) jsonnet templates
	// need to be computed during evaluation
	vm := jsonnet.MakeVM()
	output, err := vm.EvaluateAnonymousSnippet(filename, string(templateBytes))
	if err != nil {
