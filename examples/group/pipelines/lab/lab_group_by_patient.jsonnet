...
,
 ...
 "transform": {
  "cmd": [ "bash" ],
  "stdin": [ "set -x", "FILES=/pfs/group_by_patient_"+suffix+"/*/*", "for f in $FILES", "do", "directory=`dirname $f`", "out=`basename $directory`",  "cat $f >> /pfs/out/${out}.txt", "done" ]
}
}
]