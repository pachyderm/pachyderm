...
launch-dev: check-kubectl check-kubectl-connection
	...
	helm install pachyderm etc/helm/pachyderm -f etc/helm/examples/local-dev-values-with-det.yaml --version 2.7.0-test.1 --set pachd.image.tag=local --set determined.detVersion=0.23.1 --set pachd.enterpriseLicenseKey=$(ENT_ACT_CODE)
	# wait for the pachyderm to come up
	kubectl wait --for=condition=ready pod -l app=pachd --timeout=5m
	@echo "pachd launch took $$(($$(date +%s) - $(STARTTIME))) seconds"
