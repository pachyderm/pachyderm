...
def run_benchmark(env):
    if subprocess.call('kubectl run -t -i bench --image="{}" --restart=Never -- go test ./src/server -run=XXX -bench={} -timeout 60m'.format(args.pachyderm_compile_image, args.benchmark), shell=True) != 0:
        raise Failed()
