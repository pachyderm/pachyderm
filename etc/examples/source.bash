...
do_pfs_pps() {
  if [ -z ${PFS_PPS_CLIENT_EXAMPLE} ]; then
    run_make launch
    run docker run -e PFS_PPS_CLIENT_EXAMPLE=1 --link pachyderm_pfsd_1 --link pachyderm_ppsd_1 pachyderm_compile bash "etc/examples/$(basename "${1}")"
    run_make docker-clean-launch
  else
    export PFS_ADDRESS="$(echo "${PACHYDERM_PFSD_1_PORT}" | sed "s/tcp:\/\///")"
    export PPS_ADDRESS="$(echo "${PACHYDERM_PPSD_1_PORT}" | sed "s/tcp:\/\///")"
