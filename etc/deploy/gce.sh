...
parse_flags() {
  ...
  export STATE_BUCKET=gs://k8scom-state-store-pachyderm-${RANDOM}
  local USE_EXISTING_STATE_BUCKET='false'

  # Parse flags
  eval "set -- $( getopt -l "state:,zone:,project:" "--" "${0}" "${@}" )"
  while true; do
      case "${1}" in
          --state)
  ...
  if [[ ! ( "${STATE_BUCKET}" =~ gs://* ) ]]; then
    echo "kops state bucket must start with \"gs://\" but is \"${STATE_BUCKET}\""
    exit "Exiting to be safe..."
    exit 1
  fi

  if [ "${USE_EXISTING_STATE_BUCKET}" == 'false' ]; then
    create_gs_bucket "${STATE_BUCKET}" || exit 1
  fi
}

create_gs_bucket() {
  if [[ "$#" -lt 1 ]]; then
    echo "Error: create_gs_bucket needs a bucket name"
    return 1
  fi
  BUCKET="${1#gs://}"
