...
curl -X POST -H "Authorization: Bearer ${HELIUM_API_TOKEN}" \
 -F name=release-"${CIRCLE_TAG}-${CIRCLE_SHA1:0:7}" -F pachdVersion="${CIRCLE_SHA1}" -F helmVersion="${CIRCLE_TAG}-${CIRCLE_SHA1}" \
...
for _ in $(seq 108); do
  STATUS=$(curl -s -H "Authorization: Bearer ${HELIUM_API_TOKEN}" "https://helium.pachyderm.io/v1/api/workspace/release-${CIRCLE_TAG}-${CIRCLE_SHA1:0:7}" | jq .Workspace.Status | tr -d '"')
  if [[ ${STATUS} == "ready" ]]
  then
    echo "success"
    break
...
pachdIp=$(curl -s -H "Authorization: Bearer ${HELIUM_API_TOKEN}" "https://helium.pachyderm.io/v1/api/workspace/release-${CIRCLE_TAG}-${CIRCLE_SHA1:0:7}"  | jq .Workspace.PachdIp)

echo "{\"pachd_address\": ${pachdIp}, \"source\": 2}" | tr -d \\ | pachctl config set context "release-${CIRCLE_TAG}-${CIRCLE_SHA1:0:7}" --overwrite && pachctl config set active-context "release-${CIRCLE_TAG}-${CIRCLE_SHA1:0:7}"
