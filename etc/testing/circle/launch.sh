...
docker exec "$KIND_CONTAINER"  'mkdir -p /tmp/pachyderm/pachd && chmod -R 777 /tmp/pachyderm'
...
KIND_IP=$(docker inspect $KIND_CONTAINER -f 'json {{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}')
pachctl config update context "$(pachctl config get active-context)" --pachd-address="$():30650"
