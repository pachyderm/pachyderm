...
import {SetClusterDefaultsResponse, Pipeline} from '@dash-frontend/api/pps';
import {
  lineageRoute,
  pipelineRoute,
...
type AffectedPipelinesTableProps = {
  affectedPipelines: SetClusterDefaultsResponse['affectedPipelines'];
};

const projectComparator = {
  name: 'Project',
  func: stringComparator,
  accessor: (pipeline: Pipeline) => pipeline?.project?.name || '',
};

const nameComparator = {
  name: 'Name',
  func: stringComparator,
  accessor: (pipeline: Pipeline) => pipeline?.name || '',
};
