...
// StartContainer pulls image and starts a container from it with command. It returns the
...
func Materialize(in_repo, branch, commit, out_repo, jobDir string) error {
	...
	for _, jobInfo := range jobs {
		...
		containerIp, err := SpinupContainer(j.Container, j.Command)
		if err != nil {
			return err
		}
